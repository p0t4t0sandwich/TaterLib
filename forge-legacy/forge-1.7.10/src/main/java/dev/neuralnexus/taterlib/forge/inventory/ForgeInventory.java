...
public class ForgeInventory implements Inventory {
    ...
    public void removeItem(ItemStack item) {
        for (int i = 0; i < size(); i++) {
            if (item(i).type().equals(item.type())) {
                inventory.decrStackSize(i, item.count());
                return;
            }
        }
    }
    ...
    public void setContents(ItemStack[] items) {
        for (int i = 0; i < size(); i++) {
            inventory.setInventorySlotContents(i, ((ForgeItemStack) items[i]).itemStack());
        }
    }
    ...
    public void remove(String type) {
        for (int i = 0; i < size(); i++) {
            if (item(i).type().equals(type)) {
                inventory.decrStackSize(i, item(i).count());
            }
        }
    }
    ...
    public void clear(int slot) {
        inventory.decrStackSize(slot, item(slot).count());
    }
}
