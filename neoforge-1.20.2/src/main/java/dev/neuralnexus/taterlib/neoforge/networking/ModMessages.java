...
public class ModMessages {
    ...
    public static void register() {
        for (String c : channelQueue) {
            ...
            channels.put(c, NetworkRegistry.ChannelBuilder.named(
                    ...
                    .simpleChannel());
            channels.get(c).messageBuilder(NeoForgeMessagePacket.class, id(), PlayNetworkDirection.PLAY_TO_CLIENT)
    ...
    public static <MSG> void sendPluginMessage(MSG message, String channel, ServerPlayer player) {
        channels.get(channel).sendTo(message, player.connection.connection, PlayNetworkDirection.PLAY_TO_CLIENT);
    }
}
