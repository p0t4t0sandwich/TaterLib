...
public class BungeePluginMessageListener implements Listener {
    ...
    public void onPluginMessage(PluginMessageEvent event) {
        ...
        if (event.getReceiver() instanceof ProxiedPlayer) {
            PluginMessageEvents.PLAYER_PLUGIN_MESSAGE.invoke(new BungeePluginMessageEvent.Player(event));
        } else if (event.getReceiver() instanceof Server) {
            PluginMessageEvents.SERVER_PLUGIN_MESSAGE.invoke(new BungeePluginMessageEvent.Server(event));
        }
    }
}
