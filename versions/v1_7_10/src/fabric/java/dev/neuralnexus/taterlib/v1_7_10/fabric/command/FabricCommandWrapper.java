...
public class FabricCommandWrapper implements CommandCallable {
    ...
    public CommandResult process(PermissibleCommandSource source, String arguments)
        ...
        try {
            if (source instanceof EntityPlayer) {
                command.execute(new WrappedPlayer((EntityPlayer) source), command.name(), args);
            }
    ...
    @Override
    public Optional<IChatComponent> getShortDescription(PermissibleCommandSource source) {
        return Optional.empty();
    }

    @Override
    public Optional<IChatComponent> getHelp(PermissibleCommandSource source) {
        return Optional.empty();
    }

    @Override
    public IChatComponent getUsage(PermissibleCommandSource source) {
        return new ChatComponentTranslation(command.usage());
    }
}
